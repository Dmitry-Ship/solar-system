(() => {
  ...
  const VisibilityControlGroupFactory =
    namespace.application.factories.VisibilityControlGroupFactory;
  const HudView = namespace.infrastructure.dom.HudView;
  if (!VisibilityControlGroupFactory || !HudView) {
    throw new Error(
      "hud controller bootstrap failed: missing visibility control factory or HUD view."
    );
  }
